...
package it.mds.sdk.flusso.sism.residenziale.anag.controller;
...
import it.mds.sdk.flusso.sism.residenziale.anag.service.FlussoSismResServiceAnagrafica;
...
@EnableAsync
@Slf4j
public class FlussoSismResidenzialeAnagControllerRest implements ValidazioneController<RecordDtoSismResidenzialeAnagrafica> {
    ...
    private final FlussoSismResServiceAnagrafica flussoSismResServiceAnagrafica;
    private final ConfigurazioneFlussoSismResAnag conf;

    @Autowired
    public FlussoSismResidenzialeAnagControllerRest(@Qualifier("parserRegoleSismResAnag") final ParserRegole parserRegole,
                                                    @Qualifier("parserTracciatoSismResAnag") final ParserTracciato parserTracciato,
                                                    @Qualifier("flussoSismResServiceAnag") final FlussoSismResServiceAnagrafica flussoSismResServiceAnagrafica,
        ...
        this.flussoSismResServiceAnagrafica = flussoSismResServiceAnagrafica;
        this.conf = conf;
    }

    @Override
    @PostMapping(path = "v1/flusso/sismresidenzialeanagrafica")
    public ResponseEntity<RisultatoInizioValidazione> validaTracciato(@RequestBody FlussoRequest flusso, String nomeFlussoController) {
        ...
        gestoreRunLog.cambiaStatoRun(infoRun.getIdRun(), StatoRun.IN_ELABORAZIONE);
        int dimensioneBlocco = Integer.parseInt(conf.getDimensioneBlocco().getDimensioneBlocco());
        flussoSismResServiceAnagrafica.validazioneBlocchi(dimensioneBlocco, flusso.getNomeFile(), regoleFlusso, infoRun.getIdRun(),
    ...
    @Override
    public ResponseEntity<RisultatoValidazione> validaRecord(RecordRequest<RecordDtoSismResidenzialeAnagrafica> recordRequest, String nomeFlusso) {
        return null;
    }

    @Override
    @GetMapping("v1/flusso/sismresidenzialeanagrafica/info")
